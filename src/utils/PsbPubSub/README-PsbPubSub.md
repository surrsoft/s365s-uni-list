...
- `!!psb-p-method!!` - метод publish(e,p); принимает на вход идентификатор вида события `!psb-ev-id!` (e) и данные `!psb-payload!`; передаёт в каждый обработчик вида события `e` данные `p`
...
3. `!psb-p-method!` — вызывает все обработчики события, передавая типизированный `!psb-payload!`.
...
// публикуем событие с полезной нагрузкой
